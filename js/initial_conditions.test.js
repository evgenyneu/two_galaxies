...
describe('Initial conditions', () => {
  ...
  it('galaxyStarsPositionsAndVelocities', () => {
    ...
    expect(positions[0]).to.deep.closeTo(
      [3.985012495834077, 0, -0.29950024994048446], 1e-10);

    expect(positions[1]).to.deep.closeTo(
      [3.5850966520063019, 1.5, -0.25937482488824837], 1e-10);

    expect(positions[119]).to.deep.closeTo(
      [15.698317243678604, -2.6047226650039557, -1.4747508408524161], 1e-10);
    ...
    expect(velocities[1]).to.deep.closeTo(
      [-0.2872329613340334, 1.5, 0.028819424987583143], 1e-10);

    expect(velocities[119]).to.deep.closeTo(
      [0.044611774600251820, 1.2542762684421485, -0.44761077756662986e-2], 1e-10);
  });
});
